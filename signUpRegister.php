<?php
  ...
  $statement = $pdo -> prepare('SELECT username FROM bailleur WHERE username = :username');
  $statement->bindValue(':username', $username);
  $statement->execute();
  $checkUserRegister = $statement->fetch(PDO::FETCH_ASSOC);

  if(empty($checkUserRegister)){
    $statement = $pdo -> prepare("INSERT INTO bailleur (nom, prenom, contact, email, username, password) VALUES (:nom, :prenom, :contact, :email, :username, :password)");
